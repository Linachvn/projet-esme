...
df = df.withColumn(
    ...
    expr("""
        CASE 
            WHEN hour(timestamp) >= 6 AND hour(timestamp) < 12 THEN 'Matin'
            WHEN hour(timestamp) >= 12 AND hour(timestamp) < 18 THEN 'Après-midi'
            WHEN hour(timestamp) >= 18 AND hour(timestamp) < 24 THEN 'Soirée'
            ELSE 'Nuit'
        END
    """)
)
